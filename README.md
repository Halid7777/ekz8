...
.rgrade.excellent{background:linear-gradient(135deg,#34d399,#059669);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.rgrade.good{background:linear-gradient(135deg,#c084fc,#7c3aed);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.rgrade.ok{color:var(--warn)}.rgrade.fail{color:var(--wrong)}
...
.mbtn-y{padding:14px;background:linear-gradient(135deg,#34d399,#059669);
  color:#fff;font-size:14px;font-weight:700;border:none;border-radius:100px;
  cursor:pointer;touch-action:manipulation;min-height:50px}
.mbtn-y:active{opacity:0.85}
.mbtn-n{padding:14px;background:transparent;color:var(--muted);
  font-size:14px;font-weight:600;border:1.5px solid var(--border);border-radius:100px;
  cursor:pointer;touch-action:manipulation;min-height:50px}
.mbtn-n:active{background:rgba(255,255,255,0.05)}
...
<div id="start-screen" class="screen active">
  ...
  <p class="s-sub">20 случайных вопросов из базы 200 вопросов</p>
  <div class="igrid">
    ...
    <div class="ic"><div class="ic-icon">🗂</div><div class="ic-val">200</div><div class="ic-lbl">База</div></div>
...
<div class="modal-bg" id="modal">
  <div class="modal">
    ...
    <div class="modal-btns">
      <button class="mbtn-y" onclick="confirmFinish()">Да, завершить</button>
      <button class="mbtn-n" onclick="closeModal()">Продолжить</button>
    </div>
  </div>
</div>
...
var qs=[],ans=[],cur=0,tid=null,tl=TL,done=false;

function shuffle(a){return a.slice().sort(function(){return Math.random()-0.5;})}

function startQuiz(){
  var pool=shuffle(ALL_Q).slice(0,TOTAL);
  qs=pool.map(function(q){
    var sh=shuffle([0,1,2,3,4]);
    return{q:q.q,o:sh.map(function(i){return q.o[i];}),a:sh.indexOf(0)};
  });
  ans=[];for(var i=0;i<TOTAL;i++)ans.push(null);
...
function render(){
  var q=qs[cur],ua=ans[cur];
  var html='<div class="qcard"><div class="qlabel">Вопрос '+(cur+1)+' из '+TOTAL+'</div><div class="qtext">'+q.q+'</div><div class="opts">';
  for(var i=0;i<q.o.length;i++){
    var sel=(ua===i)?' sel':'';
    html+='<div class="opt'+sel+'" onclick="pick('+i+')"><span class="ol">'+L[i]+'</span><span class="ot">'+q.o[i]+'</span></div>';
  }
  html+='</div></div>';
  ...
  var nb=document.getElementById("bnxt");
...
function pick(oi){
  ...
  for(var i=0;i<opts.length;i++)opts[i].className="opt"+(i===oi?" sel":"");
}

function goNext(){
  if(cur<TOTAL-1){cur++;render();return;}
  var answered=0;for(var i=0;i<ans.length;i++)if(ans[i]!==null)answered++;
  if(answered<TOTAL){
    document.getElementById("modal-msg").textContent="Вы ответили на "+answered+" из "+TOTAL+" вопросов. Всё равно завершить?";
    document.getElementById("modal").classList.add("show");
  }else{finish();}
}

function confirmFinish(){closeModal();finish();}
function closeModal(){document.getElementById("modal").classList.remove("show");}
function goPrev(){if(cur>0){cur--;render();}}
function dismissTimeout(){document.getElementById("tovl").classList.remove("show");finish();}

function finish(){
  done=true;clearInterval(tid);
  var score=0;for(var i=0;i<TOTAL;i++)if(ans[i]===qs[i].a)score++;
  var skip=0;for(var i=0;i<TOTAL;i++)if(ans[i]===null)skip++;
  var wrong=TOTAL-score-skip;
  var pct=Math.round(score/TOTAL*100);
  var used=TL-tl,mn=Math.floor(used/60),sc=used%60;
  var g=pct>=90?"Отлично!":pct>=70?"Хорошо!":pct>=50?"Удовлетворительно":"Нужно повторить";
  var gc=pct>=90?"excellent":pct>=70?"good":pct>=50?"ok":"fail";
  ...
  document.getElementById("sgrid").innerHTML=
    ...
    '<div class="sc"><div class="sv b">'+mn+"м "+(sc<10?"0":"")+sc+"с"+'</div><div class="sl">Затрачено</div></div>';
  var rv="";
  for(var i=0;i<TOTAL;i++){
    var ua=ans[i],ca=qs[i].a,ok=(ua===ca),sk=(ua===null);
    var cc=sk?"skip":ok?"ok":"err";
    var ah=sk?'<div class="rva ask"><span class="rtag">ОТВЕТ</span><span>'+qs[i].o[ca]+'</span></div>':
      ok?'<div class="rva aok"><span class="rtag">✓</span><span>'+qs[i].o[ca]+'</span></div>':
      '<div class="rva aerr"><span class="rtag">✗ ВЫ</span><span>'+qs[i].o[ua]+'</span></div>'+
      '<div class="rva aok"><span class="rtag">✓ ВЕРНО</span><span>'+qs[i].o[ca]+'</span></div>';
    rv+='<div class="rvc '+cc+'"><div class="rvq">'+(i+1)+'. '+qs[i].q+'</div>'+ah+'</div>';
  }
  document.getElementById("rlist").innerHTML=rv;
  show("result-screen");
}
